...
/** Экран статистики автомобиля: итог, средний месяц, динамика и категории. */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun VehicleStatsScreen(
    onBack: () -> Unit,
    viewModel: VehicleStatsViewModel = viewModel(factory = VehicleStatsViewModel.Factory)
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Статистика автомобиля") },
        ...
        LazyColumn(
            ...
            item { CategoryCard(state.categorySums, state.total, currency) }
        }
    }
}

@Composable
private fun TotalCard(state: VehicleStatsState, currency: Currency) {
...
/** Разбивка по категориям с долями (суммы и проценты). */
@Composable
private fun CategoryCard(sums: List<VehicleCategorySum>, total: Double, currency: Currency) {
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            if (sums.isEmpty()) {
